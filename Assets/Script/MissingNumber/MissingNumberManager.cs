...
public class MissingNumberManager : MonoBehaviour
{
    ...
    void GenerateQuestion()
    {
        ...
        int start = Random.Range(1, maxNumber / 2); 
        int step = Random.Range(1, 4);
        int missingIndex = Random.Range(0, 4);
        ...
        sequenceStr = sequenceStr.TrimEnd(',', ' ');
    ...
    void GenerateOptions(int maxNumber)
    {
        ...
        while (options.Count < 3)
        {
            ...
            if (wrong != correctAnswer && wrong > 0 && wrong <= maxNumber && !options.Contains(wrong))
                options.Add(wrong);
        }
